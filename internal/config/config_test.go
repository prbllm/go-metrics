	...
	tests := []struct {
	...
	}{
		{
			...
			envVars: map[string]string{
				...
				CryptoKeyEnvVar:      "/tmp/env-agent-crypto.pem",
			},
			flags: []string{"-a", "flag-server:8080", "-r", "20", "-p", "10", "-k", "flag-key-456", "-crypto-key", "/tmp/flag-agent-crypto.pem"},
			expected: Config{
				ServerHost:          "env-server:9090",
				AgentReportInterval: 15 * time.Second,
				AgentPollInterval:   5 * time.Second,
				...
				DatabaseDSN:         defaultDatabaseDSN,
				Key:                 "env-key-789",
				CryptoKey:           "/tmp/env-agent-crypto.pem",
				AuditFile:           defaultAuditFile,
				AuditURL:            defaultAuditURL,
			},
	...
	tests := []struct {
	...
	}{
		{
			...
			envVars: map[string]string{
				...
				CryptoKeyEnvVar:       "/tmp/env-server-crypto.pem",
				AuditFileEnvVar:       "/tmp/env-audit.log",
				AuditURLEnvVar:        "http://localhost:8080/audit",
			},
			flags: []string{"-a", "flag-server:8080",
				...
				"-crypto-key", "/tmp/flag-server-crypto.pem",
			...
			expected: Config{
				...
				CryptoKey:           "/tmp/env-server-crypto.pem",
				AuditFile:           "/tmp/env-audit.log",
				AuditURL:            "http://localhost:8080/audit",
			},
